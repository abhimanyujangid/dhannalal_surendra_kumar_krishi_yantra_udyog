...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("bg-card text-card-foreground rounded-sm border-0 shadow-[var(--shadow-warm)]", className)}
      {...props}
    />
  );
}
...
function CardTitle({ className, ...props }: React.ComponentProps<"h3">) {
  return (
    <h3
      ...
      className={cn("text-[2rem] leading-[1.15] font-normal tracking-normal", className)}
      {...props}
    />
  );
}
