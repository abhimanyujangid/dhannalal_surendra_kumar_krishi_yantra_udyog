...
import { GALLERY_VIDEOS } from "@/features/gallery/data/gallery.data";
      ...
      {GALLERY_VIDEOS.map((item) => (
        ...
        >
          <DialogTrigger className="group relative flex h-72 w-full flex-col overflow-hidden border border-black/10 bg-white text-left">
            <DialogImage src={item.thumbnail} alt={item.title.en} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
            <span className="absolute left-4 top-4 inline-flex items-center gap-2 bg-black/70 px-3 py-1 text-xs uppercase tracking-wide text-white">
          ...
          <DialogContainer className="grid h-full w-full place-items-center">
            <DialogContent className="mx-auto h-fit w-[92%] overflow-hidden border border-[#ffb83e]/40 bg-[#fff8e8] md:w-[78%] lg:w-[68%]">
              <div className="max-h-[82vh] overflow-y-auto">
                <video className="h-[52vh] w-full object-cover" controls autoPlay preload="metadata" poster={item.thumbnail}>
